...
if ! command -v heatTransfer > /dev/null 2>&1; then
  if [ ! -d "../solver-openfoam/Make/${WM_OPTIONS}" ]; then
    ...
    echo "The executable heatTransfer is not found, but the build directory ../solver-openfoam/Make/${WM_OPTIONS} was detected. A build is probably in progress, waiting 30s..."
    sleep 30
    if ! command -v heatTransfer > /dev/null 2>&1; then
      exit 1
    fi
