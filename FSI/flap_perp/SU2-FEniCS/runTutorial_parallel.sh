...
tmux send-keys -t "$pane_su2" C-z '( time mpirun -n 2 $SU2_HOME/SU2_CFD/bin/SU2_CFD Fluid/euler_config_coupled.cfg ) 2>&1  | tee Su2.log' Enter
tmux select-pane -t "$pane_su2"
tmux select-window -t "$window"
tmux send-keys -t "$pane_FEniCS" C-z '( time python3 Solid/perp-flap.py ) 2>&1  | tee FEniCS.log' Enter 
