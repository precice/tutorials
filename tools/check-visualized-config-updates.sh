#!/bin/bash
# Run this script at the root of the repository to check if any precice-config.png needs to be updated.
# Requires that precice-config.png files have already been checked into the repository and that
...
if git status | grep precice-config.png; then
