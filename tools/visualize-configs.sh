...
for tutorial in $tutorials; do
  if [ "${tutorial}" = tools ] || [ "${tutorial}" = changelog-entries ] || [ "${tutorial}" = partitioned-heat-conduction-direct ] || [ "${tutorial}" = flow-over-heated-plate-nearest-projection ] || [ "${tutorial}" = flow-over-heated-plate-two-meshes ] || [ "${tutorial}" = partitioned-heat-conduction-complex ]; then
    continue
  else
