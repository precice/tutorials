...
For implicit-coupling runs, `precice-*-iterations.log` files are collected into `iterations-logs/` and compared by SHA-256 hash against archived reference copies stored next to each reference `.tar.gz` in a `*.iterations-logs/` directory. A successful check is logged at INFO level; a mismatch fails the test.
