...
```shell
gh workflow run run_testsuite_manual.yml -f suites=fenics_test -f build_args="PRECICE_REF:v3.0.0,OPENFOAM_ADAPTER_REF:v1.3.0,PYTHON_BINDINGS_REF:v3.0.0.0,FENICS_ADAPTER_REF:v2.1.0" -f loglevel=DEBUG --ref=develop
```
...
Run cd tools/tests
  cd tools/tests
  python systemtests.py --build_args=PRECICE_REF:v3.0.0,OPENFOAM_ADAPTER_REF:v1.3.0,PYTHON_BINDINGS_REF:v3.0.0.0,FENICS_ADAPTER_REF:v2.1.0 --suites=fenics_test --log-level=DEBUG
  cd ../../
