...
def main(inflow: 'inflow velocity' = 10,
  ...
  consF = numpy.isnan(solver.optimize('F', couplinginterface.sample('gauss', 4).integral('F_i F_i' @ ns), droptol=1e-10))
  ...
  while interface.is_coupling_ongoing():
    ...
    if interface.is_write_data_required(dt):
      F = solver.solve_linear('F', resF, constrain=consF, arguments=dict(lhs00=lhs00, lhs0=lhs0, lhs=lhs1, dt=dt, meshdofs=meshdofs, oldmeshdofs=oldmeshdofs, oldoldmeshdofs=oldoldmeshdofs, oldoldoldmeshdofs=oldoldoldmeshdofs))
      #writedata = couplingsample.eval(ns.F, F=F) # for stresses
      writedata = couplingsample.eval('F_i d:x' @ ns, F=F, meshdofs=meshdofs) * numpy.concatenate([p.weights for p in couplingsample.points])[:,numpy.newaxis]
