...
def main(inflow: 'inflow velocity' = 10,
  ...
  # define cell centers
  U, UX = couplinginterface.integrate([ns.ubasis.sum(1), ns.ubasis.sum(1)[:,numpy.newaxis] * ns.x0], degree=2)
  UX = UX.export('dense')
  where = U.nonzero()[0].reshape(2,-1).T # nx2 array of flap-supported test function indices
  XCG = UX[where] / U[where,numpy.newaxis] # nx2x2, should be constant in 2nd dimension
  assert (XCG[:,0] == XCG[:,1]).all()
  XCG = XCG[:,0]
  verticesCC = XCG
  dataIndicesCC = numpy.zeros(len(where))
  interface.set_mesh_vertices(meshIDCC, len(where), verticesCC.ravel(), dataIndicesCC)
  ...
  while interface.is_coupling_ongoing():
    ...
    writedata = R[where]
    interface.write_block_vector_data(writedataID, len(where), dataIndicesCC, writedata.ravel())
