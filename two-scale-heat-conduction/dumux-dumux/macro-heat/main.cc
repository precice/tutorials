...
int main(int argc, char **argv) {
  ...
  const precice::string_view meshNameView("macro-mesh", 9) if (argc > 2)
  ...
  if (getParam<bool>("Precice.RunWithCoupling") == true) {
    couplingParticipant.writeQuantityVector(meshNameView, writeDataTemperature,
                                            temperatures);
    ...
    couplingParticipant.writeQuantityVector(meshNameView, readDatak00,
                                            kInitial);
    couplingParticipant.writeQuantityVector(meshNameView, readDatak01,
                                            kInitial);
    couplingParticipant.writeQuantityVector(meshNameView, readDatak10,
                                            kInitial);
    couplingParticipant.writeQuantityVector(meshNameView, readDatak11,
                                            kInitial);
    couplingParticipant.writeQuantityVector(meshNameView, readDataPorosity,
                                            porosityInitial);
  }
}
...
do {
  if (getParam<bool>("Precice.RunWithCoupling") == true) {
    ...
    couplingParticipant.readQuantityFromOtherSolver(meshNameView, readDatak00,
                                                    dt);
    couplingParticipant.readQuantityFromOtherSolver(meshNameView, readDatak01,
                                                    dt);
    couplingParticipant.readQuantityFromOtherSolver(meshNameView, readDatak10,
                                                    dt);
    couplingParticipant.readQuantityFromOtherSolver(meshNameView, readDatak11,
                                                    dt);
  ...
  if (getParam<bool>("Precice.RunWithCoupling") == true) {
    ...
    dt = std::min(nonLinearSolver.suggestTimeStepSize(timeLoop->timeStepSize()),
                  getParam<Scalar>("TimeLoop.MaxDt"));
