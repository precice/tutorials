...
usage() {
  cat >&2 <<'EOF'
Usage:
  script.sh [--exec=/path/to/SedimentingBlock] [--build-dir=/path/to/mercurydpm/build]

Options:
  -e=, --exec=        Explicit executable path to run or
  -b=, --build-dir=   MercuryDPM build dir (root). Script will look for:
                      <build-dir>/Drivers/PreCICE/SedimentingBlock
EOF
}
...
EXE="SedimentingBlock"

# 2) MercuryDPM build dir
if [[ -n "${MERCURYDPM_BUILD_DIR}" ]]; then
  # location in the build dir:
  # /path/to/mercurydpm/build/Drivers/PreCICE/SedimentingBlock
