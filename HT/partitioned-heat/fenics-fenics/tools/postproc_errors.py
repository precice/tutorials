...
for d_D, d_N in zip(data_D[1:],data_N[1:]):
    ...
    try:
        structured_data[wrcplkey][dTkey] = np.max([d_D["error"], d_N["error"]])
...
for wr, cpl in itertools.product(evaluated_wr, coupling_schemes):
    ...
    plt.loglog(evaluated_dT, [structured_data[wrcplkey][dtkey] for dtkey in keys], label=wrcplkey, marker=next(marker))
print(tabulate(table[1:], headers = table[0],tablefmt="latex_booktabs", floatfmt=".2e"))
plt.legend()
plt.grid()
plt.show()
